...
onLoadCompleted() {
    ...
    $('#preloader .icon').remove();
    $('#preloader .title').text('Bugatti Chiron Sport');
    $('#preloader .desc').html('The breathtaking new interpretation of the ultimate hypersports car redefined.');

    // Change background from image to video
    $('#preloader').css({
        'background': 'none', // remove the current image background
        'background-color': 'rgba(0, 0, 0, 0.7)' // maintain the overlay effect
    });

    // Append video element
    $('#preloader').append(`
        <video id="preloader-video" autoplay muted loop style="position: absolute; width: 100%; height: 100%; object-fit: cover; z-index: -1;">
